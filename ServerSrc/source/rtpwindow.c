...
int fillWinBuffer(char * s)
{
    ...
    dataLength = (int)strlen(s);
    spaceLeft = MSG_BUFFER_SIZE-buffReadPos;
    ...
    if(dataLength > spaceLeft)
    {
        ...
        strncpy(buffer + buffReadPos, s, (size_t)spaceLeft);
        buffer[MSG_BUFFER_SIZE-2] = '\n';
        buffer[MSG_BUFFER_SIZE-1] = '\0';
        return -1;
    }

    strncpy(buffer + buffReadPos, s, (size_t)dataLength);
...
void winMsgReceived()
{
    /* Send only ACK if message already received */
